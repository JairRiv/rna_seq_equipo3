...
Tras el análisis transcriptómico, se encontró un cambio en la expresión de genes asociados a la producción de antioxidantes, compensando el agotamiento de compuestos de azufre.
...
Se realizó una normalización de Transcritos por Millón (TPM), para posteriormente generar un PCA. El primer gráfico mostró que los datos ya presentaban una estructura coherente; los clusters correspondientes a las condiciones GC, A1G y MG eran claramente distinguibles, lo que indicaba una separación biológica sólida desde el comienzo. 
En este punto, la primera componente principal (PC1) lograba explicar el 33% de la varianza total del experimento.
Con el objetivo de optimizar la precisión del análisis, se procedió a realizar una corrección de batch effect, por medio de la librería Limma.
Aunque el agrupamiento inicial era aceptable, tras este proceso de corrección, los clusters en el PCA se visualizaron de manera mucho más compacta y definida. Con esto se demuestra que la variación tenga una explicación biológica y no se deba a ruido o a variación en las corridas de secuenciación.
La efectividad de esta corrección se observa en el aumento de la varianza explicada por los componentes: el valor de la PC1 ascendió del 33.3% al 46%, lo que demuestra que, tras eliminar el ruido técnico, la componente principal captura una mayor proporción de la variabilidad biológica.
Es importante notar que, si bien el scree plot todavía muestra los mismos valores en la varianza, el análisis del PCA corregido confirma visualmente una mejora en la agrupación de los datos.
...
Al comparar dichos resultados con otros artículos e investigaciones, se encontraron conclusiones similares. En el artículo de Vinken, se encontró cómo los viajes espaciales provocan cambios negativos en la función hepática: inflamación y estrés oxidativo. Se confirma una reducción en los niveles de antioxidantes de azufre. 
Otro estudio, de Blaber et. al (2017) utilizó ratones para demostrar que los viajes espaciales provocan mayor estrés oxidativo en los organismos. Ambos artículos mencionan al gen Gclc, como parte de la respuesta compensatoria a dicho estrés.
